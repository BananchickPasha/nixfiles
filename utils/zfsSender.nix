{ config, pkgs, lib, ... }:
let
  wrapScriptFunc = pkgs.helpers.scriptWriter;
  zfsNoSync =
    wrapScriptFunc /etc/nixos/sources/zfsSender/zfsNoSync.sh [ pkgs.zfs ];
...
in {
  ...
  config = let
    systemdName = lib.stringAsChars (char: if char == "/" then "_" else char);
  in lib.mkIf (cfg.sendFromToPairs != [ ]) {
    systemd.services = builtins.listToAttrs (map (pair: {
      name = "zfs-snapshot-sender-from-${systemdName pair.from}-to-${
          systemdName pair.to
        }";
      value = {
        ...
        serviceConfig = {
          Type = "oneshot";
          User = "root";
          ExecStart = if pair.sync then
            "${zfsSync}/zfsSync.sh ${pair.from} ${pair.to}"
          else
            "${zfsNoSync}/zfsNoSync.sh ${pair.from} ${pair.to}";
        };
      };
    }) cfg.sendFromToPairs);

    systemd.timers =
      let time = x: (if cfg.customPeriod && x != "" then x else "hourly");
      in builtins.listToAttrs (map (pair: {
        name = "zfs-snapshot-sender-from-${systemdName pair.from}-to-${
            systemdName pair.to
          }";
        value = {
